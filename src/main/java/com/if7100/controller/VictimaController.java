...
public class VictimaController {
	...
	public void exportToPDF(HttpServletResponse response) throws IOException, java.io.IOException {
		...
		for (Victima victima : victimas) {
			...
			table.addCell(new Cell()
					.add(new Paragraph(victima.getCVNombre() + " " + victima.getCVApellidoPaterno() + " "
							+ victima.getCVApellidoMaterno()))
					.setTextAlignment(TextAlignment.CENTER));
			...
			table.addCell(new Cell()
					.add(new Paragraph(
							identidadGeneroService.getIdentidadGeneroById(victima.getCVGenero()).getNombre()))
					.setTextAlignment(TextAlignment.CENTER));

			table.addCell(new Cell()
					.add(new Paragraph(orientacionSexualService.getOrientacionSexualByCodigo(victima.getCVOrientaSex())
							.getCVTitulo()))//
					.setTextAlignment(TextAlignment.CENTER));
	...
	public void exportToExcel(HttpServletResponse response) throws IOException, java.io.IOException {
		...
		String[] headers = { "ID", "DNI", "Nombre", "Primer apellido", "Segundo apellido",
				"Edad", "Género",
				"Lugar de nacimiento", "Orientación sexual", "Nacionalidad", "Educación", "Ocupación", "Domicilio",
				"Lugar de residencia",
				"Discapacidad", "Condición migratoria", "Etnia", "Medidas de protección", "Denuncia previa", "Hijos",
				"Organismo generador" };
		...
		for (Victima victima : victimas) {
			...
			// Generador
			row.createCell(6)
					.setCellValue(identidadGeneroService.getIdentidadGeneroById(victima.getCVGenero()).getNombre());
			...
			row.createCell(8).setCellValue(
					orientacionSexualService.getOrientacionSexualByCodigo(victima.getCVOrientaSex()).getCVTitulo());
			...
			// Observaciones
			row.createCell(10)
					.setCellValue(nivelEducativoService.getNivelEducativoById(victima.getCIEducacion()).getCVTitulo());
	...
	public String listVictima(Model model, @PathVariable Integer pg) {
		...
		/*
		 * List<Integer> nPaginas = IntStream.rangeClosed(1,
		 * victimaPage.getTotalPages())
		 * .boxed()
		 * .toList();
		 */
	...
	public String updateVictima(@PathVariable Integer id,
		...
		// existingVictima.setCodigoPais(victima.getCodigoPais());//actualiza codigo
		// pais
